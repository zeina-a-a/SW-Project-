...
class UserController
{
    ...
    public function upgradeUser($id, $connectionCount)
    {
        if ($connectionCount !== null && $connectionCount >= 5) {
            ...
            if ($result) {
                (Email::sendEmail($_SESSION["email"], 'You Are Premium!'));
    ...
    // public function incrementConnection($userId)
    // {
    //     $result = $this->_userRepository->incrementConnectionCountQuery($userId);
    //     if (!$result) {
    //         return false;
    //     }
    //     return true;
